...
def authenticate_user(id_empresa:int,full_name: str,puesto_trabajo, password: str, db: Session):
    user = db.query(Users).filter(Users.full_name == full_name, Users.id_empresa == id_empresa, Users.puesto_trabajo == puesto_trabajo).first()
    if not user:
        return False
